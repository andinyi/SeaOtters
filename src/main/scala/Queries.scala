class Queries {
  ...
  def query10():String = {
    "SELECT t1.location, Pre_Vaccine, Post_Vaccine, (Post_Vaccine-Pre_Vaccine) AS diff FROM "+
      ...
      "FROM data WHERE NOT continent = '' AND NOT new_vaccinations_smoothed = 'tests performed' AND NOT Int(new_vaccinations_smoothed) IS Null AND date < '2022-06-01' GROUP BY location)) AS t1 "+
      "INNER JOIN " +
      ...
      "FROM data WHERE NOT continent = '' AND Int(new_vaccinations_smoothed) is NULL AND date < '2022-06-01' GROUP BY location)) AS t2 "+
